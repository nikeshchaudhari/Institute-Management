...
router.get("/all-student", checkAuth, async (req, res) => {
  try {
    const token = await req.headers.authorization.split(" ")[1];
    const verifyToken = await jwt.verify(token, "secret key");
    const allStudent = await Student.find({ uId: verifyToken.uId }).select(
...
router.get("/recent",checkAuth,async(req,res)=>{
 ...
 try{
    const token =  await req.headers.authorization.split(" ")[1]
    const verifyToken = await jwt.verify(token,"scret key")
 
 const recentData = Student.find({uId:verifyToken.uId})
